...
#
proc isppipeline_gen_reset_gpio {drv_handle node dts_file} {
...
proc isppipeline_generate {drv_handle} {
	...
	add_prop $node "xlnx,bgain" "/bits/ 16 <210>" noformating $dts_file
	add_prop $node "xlnx,pawb" "/bits/ 16 <350>" noformating $dts_file
	add_prop $node "xlnx,mode-reg" "" boolean $dts_file
	isppipeline_gen_reset_gpio "$drv_handle" "$node" $dts_file
	# generating ports node for ispipeline ip
	set isppipeline_ports_node [create_node -n "ports" -l isppipeline_ports$drv_handle -p $node -d $dts_file]
	add_prop "$isppipeline_ports_node" "#address-cells" 1 int $dts_file
	...
	if {[llength $inip]} {
		...
		add_prop "$isppipeline_port_node_endpoint" "remote-endpoint" $inip$drv_handle reference $dts_file
	}
	...
	set port1_node [create_node -n "port" -l isppipeline_port1$drv_handle -u 1 -p $isppipeline_ports_node -d $dts_file]
